...
async function main() {
    const serviceName = process.argv[2] || null;
    const phoneNumber = '+15556742404';
    const customerName = 'Joyce';
    const bookingDate = '2025-12-15';
    const bookingTime = '12:00';
    const whatsappId = phoneNumber.replace(/[^0-9]/g, '');
    const conversation = [
        {
    ...
    try {
        ...
        if (!selectedService) {
            ...
            if (!packageExists) {
                const packages = await prisma.package.findMany();
                throw new Error(`Service "${selectedService}" not found. Available packages: ${packages.map(p => p.name).join(', ')}`);
            }
            console.log(`\n✅ Using service: ${selectedService}`);
        }
        const STUDIO_TZ = 'Africa/Nairobi';
        const bookingDateTimeLocal = luxon_1.DateTime.fromISO(`${bookingDate}T${bookingTime}`, {
            zone: STUDIO_TZ,
        });
...
//# sourceMappingURL=import-conversation-booking.js.map