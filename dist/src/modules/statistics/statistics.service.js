...
let StatisticsService = class StatisticsService {
    ...
    async getCustomerEmotionsStats(startDate, endDate) {
        const where = {};
        ...
        const customersWithDetails = await Promise.all(negativeSentimentCustomers.map(async (item) => {
    ...
    async getEmotionalToneStats(startDate, endDate) {
        const where = {};
        if (startDate || endDate) {
            where.createdAt = {};
            if (startDate)
    ...
    async getPersonalizedResponseStats(startDate, endDate) {
        const where = {};
        if (startDate || endDate) {
            where.createdAt = {};
            if (startDate)
    ...
    async getAIPerformanceStats(startDate, endDate) {
        const where = {};
        if (startDate || endDate) {
            where.timestamp = {};
            if (startDate)
        ...
        if (startDate || endDate) {
            ...
            if (startDate)
                learningWhere.createdAt.gte = startDate;
            if (endDate)
                learningWhere.createdAt.lte = endDate;
        }
        const learnings = await this.prisma.conversationLearning.findMany({
            where: learningWhere,
        });
    ...
    async getSystemStats(startDate, endDate) {
        ...
        const [totalCustomers, activeCustomers, customersWithBookings, newCustomers, aiEnabledCustomers,] = await Promise.all([
            ...
            this.prisma.customer.count({
                where: where.createdAt ? where : { createdAt: { gte: (0, date_fns_1.subDays)(new Date(), 30) } },
            }),
            this.prisma.customer.count({
                where: { aiEnabled: true },
            }),
        ]);
        const [totalMessages, inboundMessages, outboundMessages, messagesByPlatform,] = await Promise.all([
        ...
        const [totalBookings, confirmedBookings, completedBookings, cancelledBookings, bookingsByStatus,] = await Promise.all([
        ...
        const [totalEscalations, openEscalations, resolvedEscalations, escalationsByType,] = await Promise.all([
        ...
        const [totalChurnAlerts, activeChurnAlerts, churnAlertsByRiskLevel,] = await Promise.all([
        ...
        if (startDate || endDate) {
            ...
            if (startDate)
                predictionWhere.timestamp.gte = startDate;
            if (endDate)
                predictionWhere.timestamp.lte = endDate;
        }
        ...
        if (startDate || endDate) {
            ...
            if (startDate)
                conversationWhere.timestamp.gte = startDate;
            if (endDate)
                conversationWhere.timestamp.lte = endDate;
        }
        ...
        return {
            ...
            period: {
                start: startDate || (0, date_fns_1.subDays)(new Date(), 30),
                end: endDate || new Date(),
            },
        };
    }
    async getComprehensiveStats(startDate, endDate) {
        const [systemStats, customerEmotions, emotionalTones, personalizedResponses, aiPerformance,] = await Promise.all([
